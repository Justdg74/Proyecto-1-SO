...
public class Dashboard extends javax.swing.JFrame {
                ...
                if(!path.contains("txt")) {
                    ...
                    try{
                        ...
                        if(!"".equals(complete)) {
                            ...
                            else if (infocom1[0].compareTo("Apple")==0) {
                                infocom1[1]=infocom1[1].trim();
                                String [] divTra=infocom1[1].split(":");
                                divTra[1]=divTra[1].trim();
                                String [] trabajadores=divTra[1].split("\n");
                            ...
                            else if (infocom2[0].compareTo("Apple")==0) {
    ...
    private void SaveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_SaveButtonActionPerformed
        if(iniciado == true){
            //MSI
            int placasMsi=(int) this.Cont_Placas_MSI.getValue();
            int cpusMsi=(int) this.Cont_CPU_MSI.getValue();
            int ramsM=(int) this.Cont_RAM_MSI.getValue();
            int fuenteAlMsi=(int) this.Cont_FuenteA_MSI.getValue();
            int tarjetasMsi=(int) this.Cont_Tarjetas_MSI.getValue();
            int ensamCN=(int) this.Cont_Ensamblador_MSI.getValue();
            //APPLE
            int placasAp=(int) this.Cont_Placas_Apple.getValue();
            int cpusAp=(int) this.Cont_CPU_Apple.getValue();
            int ramsAp=(int) this.Cont_RAM_Apple.getValue();
            int fuenteAlAp=(int) this.Cont_FuenteA_Apple.getValue();
            int tarjetasAp=(int) this.Cont_Tarjetas_Apple.getValue();
            int ensamN=(int) this.Cont_Ensamblador_Apple.getValue();
            
            int deadline=(int) this.Cont_Deadline.getValue();
            int DuracionD=(int) this.Cont_Day_Duration.getValue();
            
            String TodoTXT="Duracion\n"+DuracionD+";\nDeadline\n"+deadline+";\nMsi-\nTrabajadores:\nplacas,"+placasMsi+
                    "\ncpus,"+cpusMsi+"\nrams,"+ramsM+"\nfuentesal,"+fuenteAlMsi+"\ntarjetasgraficas,"+tarjetasMsi+"\nensambladores,"+ensamCN+";\n"
                    + "Apple-\nTrabajadores:\nplacas,"+placasAp+
                    "\ncpus,"+cpusAp+"\nrams,"+ramsAp+"\nfuentesal,"+fuenteAlAp+"\ntarjetasgraficas,"+tarjetasAp+"\nensambladores,"+ensamN;
